...
export default function Handle({ route, navigation }) {
  ...
  useEffect(() => {
    axios
      .post(`${API}/api/daegu/toilet/gethandle`, {
        team_skey: teamKey,
        list_skey: listKey,
      })
      .then((res) => {
  ...
  const DataSave = () => {
    ...
    uploadImgToGcs(image, regionKey, region, listKey, dataCollection, data)
      .then((result) => {
        axios
          .post(`${API}/api/daegu/toilet/sethandle`, {
            team_skey: teamKey,
            list_skey: listKey,
            to_handle_YN: value.to_handle_YN,
            to_handle_toilet_YN: value.to_handle_toilet_YN,
            to_handle_basin_YN: value.to_handle_basin_YN,
          })
          .then((res) => {
            const response = JSON.parse(res.data);
  ...
  const handleOnSubmit = async () => {
    if (value.to_handle_YN === null || value.to_handle_toilet_YN === null || value.to_handle_basin_YN === null) {
      Alert.alert("모든 항목을 입력해주세요.");
    } else if (value.to_handle_YN === "Y" && imageLength === 0) {
