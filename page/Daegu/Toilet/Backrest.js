...
export default function Backrest({ route, navigation }) {
  ...
  useEffect(() => {
    axios
      .post(`${API}/api/daegu/toilet/getbackrest`, {
        team_skey: teamKey,
        list_skey: listKey,
      })
      .then((res) => {
  ...
  const DataSave = () => {
    ...
    uploadImgToGcs(image, regionKey, region, listKey, dataCollection, data)
      .then((result) => {
        axios
          .post(`${API}/api/daegu/toilet/setbackrest`, {
            team_skey: teamKey,
            list_skey: listKey,
            to_toilet_backrest_YN: value.to_toilet_backrest_YN,
          })
          .then((res) => {
            const response = JSON.parse(res.data);
  ...
  const handleOnSubmit = async () => {
    if (value.to_toilet_backrest_YN === null) {
      Alert.alert("모든 항목을 입력해주세요.");
    } else if (value.to_toilet_backrest_YN === "Y" && imageLength === 0) {
          ...
          <View style={styles.add}>
            <View style={styles.add_wrapper}>
              <RadioBtn
                title="변기 등받이 유무"
                getCheck={getCheck}
                name="to_toilet_backrest_YN"
                value={value.to_toilet_backrest_YN}
                yes="있다"
                no="없다"
              />
