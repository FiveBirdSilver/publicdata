...
export default function Doorway({ route, navigation }) {
  ...
  useEffect(() => {
    axios
      .post(`${API}/api/daegu/toilet/getdoorway`, {
        team_skey: teamKey,
        list_skey: listKey,
      })
      .then((res) => {
  ...
  const DataSave = () => {
    ...
    uploadImgToGcs(image, regionKey, region, listKey, dataCollection, data)
      .then((result) => {
        axios
          .post(`${API}/api/daegu/toilet/setdoorway`, {
            team_skey: teamKey,
            list_skey: listKey,
            to_dw_sill_YN: value.to_dw_sill_YN,
            to_dw_width: value.to_dw_width,
            to_dw_sill_height: value.to_dw_sill_height,
          })
          .then((res) => {
            const response = JSON.parse(res.data);
            ...
            <View style={styles.add_wrapper}>
              ...
              <View style={{ position: "relative" }}>
                <Input
                  title="턱 높이"
                  getText={getText}
                  name="to_dw_sill_height"
                  value={value.to_dw_sill_height}
                  keyboardType={"numeric"}
                />
                <Text style={{ position: "absolute", top: 13, right: 10 }}>cm</Text>
              </View>
              ...
              <View style={styles.img}>
                <TakePhoto title="사진 1" name="d_t_dw_photo1" getImage={getImage} value={value.d_t_dw_photo1} />
                <TakePhoto title="사진 2" name="d_t_dw_photo2" getImage={getImage} value={value.d_t_dw_photo2} />
              </View>
            </View>
          </View>
        </View>
